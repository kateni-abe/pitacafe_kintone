(() => {
  ...
  // 対応するイベント：新規作成、編集画面、さらに「一覧画面での編集」を追加
  const events = [
    `app.record.create.change.${DROPDOWN_CODE}`,
    `app.record.edit.change.${DROPDOWN_CODE}`,
    `app.record.index.edit.change.${DROPDOWN_CODE}` // ← これが一覧画面用！
  ];
  ...
  kintone.events.on(events, (event) => {
    ...
    console.log('--- pitalab カスタマイズ（一覧/詳細対応版）実行 ---');
