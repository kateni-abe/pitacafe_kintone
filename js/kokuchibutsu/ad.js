(function () {
  ...
  function tagView(setInfo, buttons) {
    ...
    const fieldsForA = ['利用目的'];
    const fieldsForB = ['修正ログ'];
    const fieldsForC = ['完成データ'];

    // 管理対象の全フィールドコードの重複をなくしたリストを作成
    const allFields = [...new Set([...fieldsForA, ...fieldsForB, ...fieldsForC])];
  ...
  kintone.events.on(['app.record.create.show', 'app.record.edit.show', 'app.record.detail.show'], function (e) {
    // --- ボタンの生成 ---
    const buttons = {
      ButtonA: createButton('ButtonA', '利用目的・制作仕様'),
      ButtonB: createButton('ButtonB', '修正ログ'),
      ButtonC: createButton('ButtonC', '完成データ'),
    ...
    if (tabSpace) {
      if (tabSpace.children.length === 0) { // 二重追加を防止
        ...
        tabSpace.appendChild(buttons.ButtonC);
